...
import dontmanage
from dontmanage.core.doctype.file.file import get_content_hash, get_file_name
from dontmanage.model.document import Document


# www/docs/user/manual/en/accounts/bank-reconciliation.md
class MigrateToWiki(Document):
	# app_name = dontmanageerp_documentation
	...
	def create_first_path(self):
		...
		wiki_sidebar = dontmanage.new_doc("Wiki Sidebar")
		wiki_sidebar_dict = {
	...
	def set_docs_tree_generator(self):
		self.docs_tree_generator = os.walk(
			f"{dontmanage.get_app_path(self.app_name)}{os.sep}{self.docs_directory}"
		)

	def set_assets_tree_generator(self):
		self.assets_tree_generator = os.walk(
			dontmanage.get_app_path(self.app_name) + os.sep + self.assets_directory
		)
	...
	def migrate_dir(self, root, dirs, files):
		for directory in dirs:
			...
			wiki_sidebar_item = dontmanage.new_doc("Wiki Sidebar Item")
	...
	def migrate_file(self, root, file, files):
		...
		wiki_page = dontmanage.new_doc("Wiki Page")
		wiki_page_dict = {
			"title": title,
			"allow_guest": 1,
			"published": 1,
	...
	def copy_assets(self):
		shutil.copytree(
			dontmanage.get_app_path(self.app_name) + os.sep + self.assets_directory,
			f"{os.getcwd()}{os.sep}{dontmanage.local.site}{os.sep}public{os.sep}files{os.sep}{self.app_name}{os.sep}{self.documentation_route}",
		)
		...
		for root, dirs, files in self.assets_tree_generator:
			...
			for file in files:
				if file == "__init__.py":
					continue
				if os.path.exists(
					f"{os.getcwd()}{os.sep}{dontmanage.local.site}{os.sep}public{os.sep}files{os.sep}{file}"
				):
					if self.create_new_assets:
						try:
							...
							dontmanage.msgprint(dontmanage._("File {0} does not exist").format(f"{root}{os.sep}{file}"))
							raise

						new_file_name = get_file_name(file, content_hash[-6:])
						shutil.copy(
							f"{root}{os.sep}{file}",
							f"{os.getcwd()}{os.sep}{dontmanage.local.site}{os.sep}public{os.sep}files{os.sep}{new_file_name}",
						)
					...
					shutil.copy(
						f"{root}{os.sep}{file}",
						f"{os.getcwd()}{os.sep}{dontmanage.local.site}{os.sep}public{os.sep}files{os.sep}",
					)
					file_url = f"{os.sep}files{os.sep}{file}"
