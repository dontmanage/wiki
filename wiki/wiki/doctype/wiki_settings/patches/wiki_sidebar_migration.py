...
def execute():
	...
	if not hasattr(wiki_settings, "sidebar") or not dontmanage.get_all(
		"Wiki Sidebar", pluck="name", order_by="creation"
	):
		return
	topmost = find_topmost(wiki_settings.sidebar)

	dontmanage.reload_doctype("Wiki Sidebar")
	sidebar_items = get_children(dontmanage.get_doc("Wiki Sidebar", topmost))
	sidebars = get_sidebar_for_patch(sidebar_items, topmost)

	# store sidebars in wiki settings
	sidebar_items = sidebars.items()
	dontmanage.reload_doctype("Wiki Settings")
	if sidebar_items:
		for sidebar, items in sidebar_items:
			...
			dontmanage.db.delete("Wiki Sidebar", sidebar)
	wiki_settings.wiki_search_scope = topmost
	wiki_settings.save()


def find_topmost(me):
	parent = dontmanage.db.get_value("Wiki Sidebar Item", {"item": me, "type": "Wiki Sidebar"}, "parent")
	if not parent:
		return me
...
def get_children(doc):
	...
	for idx, sidebar_item in enumerate(out):
		if sidebar_item.type == "Wiki Sidebar":
			sidebar = dontmanage.get_doc("Wiki Sidebar", sidebar_item.item)
...
def get_sidebar_items(doc):
	...
	for item in items:
		item.group_name = dontmanage.get_doc("Wiki Sidebar", item.parent).title
